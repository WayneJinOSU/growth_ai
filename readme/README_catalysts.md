...
### 模块 2: 二级催化 — 硬事件 (Hard Events — V3.5 增强)

**核心问题:** 未来 3-6 个月有哪些具体的验证节点？是否有"供给侧灾难"？

**逻辑:**
1. 通过 `get_press_releases` 获取官方公告
2. LLM 识别两类事件:
   - **Recently Triggered**: 过去 30 天内的高影响事件 (仍在驱动价格)
   - **Upcoming**: 未来 3-6 个月的预期催化事件（常规财报、新品发布等）
3. 事件类型:
   - 需求侧: 财报发布、产品发布/换代、投资者日
   - **供给侧 (V3.5 新增):** Secondary Offerings (二次增发)、Convertible Notes (可转债)、IPO Lock-up Expiry (解禁)
4. **时间戳卫士 (V3.5):** 事件被明确赋予有效期约束：
   - 需求侧事件/常规产品预期：**未来 3-6 个月**
   - 供给侧事件 (增发/解禁)：仅限**过去 30 天**或**未来 3 个月**内 (过滤陈年噪音)
5. 每个事件必须附带具体日期或季度
6. 二次 LLM 调用分析催化事件的影响力和预期市场反应
